...
//  arma::vec alphaOR = param.subvec(2*J+1,3*J);
  ...
  arma::vec Grad(1 + 3*Q + 3*p);
  arma::vec iGrad(1 + 3*Q + 3*p);
  arma::mat GradSquare(1 + 3*Q + 3*p,1 + 3*Q + 3*p);
  Grad.fill(0);
  iGrad.fill(0);
  GradSquare.fill(0);
  ...
  for (int j = 0; j < J; ++j)
  {
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (riskT(i,j)==0) {
          ...
          if (riskNT(i,j)==0) {
            ...
            if(YT(i,j)==1) {
              ...
              {
              x = X(i,k);
              iGrad[3*Q + p + k + 1] = x*(1-iProbTafterNT);
              }}
            else {
              ...
              {
                x = X(i,k);
                iGrad[3*Q + p + k + 1] = -x*iProbTafterNT;
              }
            }
          } else {
            ...
            if (iOR>0.9999 & iOR<1.0001)
            {
              ...
              // Rcpp::Rcout << "Small OR:  "<< iOR << std::endl;
               if (YNT(i,j)==1 && YT(i,j)==1) {
                 ...
                 {
                   x = X(i,k);
                   iGrad[3*Q + k + 1] = x*(1 - iProb1);
                   iGrad[3*Q + p + k + 1] = x*(1 - iProb2);
                   iGrad[3*Q + 2*p + k + 1] = 0;
                 }}
            ...
            if (YNT(i,j)==1 && YT(i,j)==1) {
              ...
              {
                x = X(i,k);
                iGrad[3*Q + k + 1] = 0.5*x*iProb1*(1 - iProb1) * (nuij - 1 - cij + 2*iOR*iProb2)/(nuij*iProb12);
                iGrad[3*Q + p + k + 1] = 0.5*x*iProb2*(1 - iProb2) * (nuij - 1 - cij + 2*iOR*iProb1)/(nuij*iProb12);
                iGrad[3*Q + 2*p + k + 1] = x*iOR*
                  (((iOR-1)*((iProb1 + iProb2) - ((1+cij)*(iProb1 + iProb2) - 2*iProb1*iProb2*(2*iOR-1))/nuij)) - 1 - cij + nuij ) /
                  (2*(iOR - 1)*(iOR - 1)*iProb12);
              }}
            if (YNT(i,j)==1 && YT(i,j)==0) {
      ...
      {
        x = X(i,k);
      iGrad[3*Q + k + 1] = ((x*iProb1*(1 - iProb1)/(iProb1 - iProb12))) *(1 -  0.5*(nuij - 1 - cij + 2*iOR*iProb2)/nuij);
      iGrad[3*Q + p + k + 1] = -0.5*x*iProb2*(1 - iProb2) * (nuij - 1 - cij + 2*iOR*iProb1)/(nuij*(iProb1 - iProb12));
      iGrad[3*Q + 2*p + k + 1] =  -x*iOR*
        (((iOR-1)*((iProb1 + iProb2) - ((1+cij)*(iProb1 + iProb2) - 2*iProb1*iProb2*(2*iOR-1))/nuij)) - 1 - cij + nuij ) /
         (2*(iOR - 1)*(iOR - 1)*(iProb1 - iProb12));
      }}
      if (YNT(i,j)==0 && YT(i,j)==1) {
        ...
        {
      x = X(i,k);
      iGrad[3*Q + k + 1] = -0.5*x*iProb1*(1 - iProb1) * (nuij - 1 - cij + 2*iOR*iProb2)/(nuij*(iProb2 - iProb12));
      iGrad[3*Q + p + k + 1] = ((x*iProb2*(1 - iProb2)/(iProb2 - iProb12))) *(1 -  0.5*(nuij - 1 - cij + 2*iOR*iProb1)/nuij);
      iGrad[3*Q + 2*p + k + 1] =  -x*iOR*
        (((iOR-1)*((iProb1 + iProb2) - ((1+cij)*(iProb1 + iProb2) - 2*iProb1*iProb2*(2*iOR-1))/nuij)) - 1 - cij + nuij ) /
          (2*(iOR - 1)*(iOR - 1)*(iProb2 - iProb12));
              }
            ...
            if (YNT(i,j)==0 && YT(i,j)==0) {
              ...
              {
                x = X(i,k);
      iGrad[3*Q + k + 1] = ((x*iProb1*(1 - iProb1)/(1 - iProb1 - iProb2 + iProb12))) *(0.5*(nuij - 1 - cij + 2*iOR*iProb2)/nuij - 1);
      iGrad[3*Q + p + k + 1] = ((x*iProb2*(1 - iProb2)/(1 - iProb1 - iProb2 + iProb12))) *(0.5*(nuij - 1 - cij + 2*iOR*iProb1)/nuij - 1);
      iGrad[3*Q + 2*p + k + 1] =  x*iOR*
        (((iOR-1)*((iProb1 + iProb2) - ((1+cij)*(iProb1 + iProb2) - 2*iProb1*iProb2*(2*iOR-1))/nuij)) - 1 - cij + nuij ) /
          (2*(iOR - 1)*(iOR - 1)*(1 - iProb1 - iProb2 + iProb12));
              }
