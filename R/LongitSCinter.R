...
#' @author Daniel Nevo
#' @export
LongitSCinter <- function(longit.data, times = NULL, formula.NT, formula.T, formula.OR = NULL, 
                          formula.inter, data, epsOR = 10^(-10),
                          knots = NULL, lambda = 0, init = NULL, maxit.optim = 50000)
{
  ...
  XinterMat <- as.matrix(model.matrix(formula.inter, data = data)[, -1])
  ...
  if (is.null(init))
  {
    ...
    res.opt <- optim(par = init, fn = PenalLogLikWithInter, gr = GradPenalLogLikWithInter, 
    ...
    hess.no.penal <- numDeriv::jacobian(func = GradPenalLogLikWithInter, x = res.opt$par, epsOR = epsOR,
                                        XNT = XNTmat, XT = XTmat, XOR = XORmat,
                                        XinterMat = XinterMat,
                                        YNT = longit.data$YNT, YT = longit.data$YT, 
                                        riskNT = longit.data$risk.NT, riskT = longit.data$risk.T,
    ...
    fit$coef.inter <- fit$est[(1 + 3*Q + pNT + pT + pOR + 1):(1 + 3*Q + pNT + pT + pOR + pInter)]
    ...
    fit$se.rob.inter <- fit$se.rob[(1 + 3*Q + pNT + pT + pOR + 1):(1 + 3*Q + pNT + pT + pOR + pInter)]
  ...
  if (pInter==1) {
    names(fit$coef.inter) <- names(fit$se.rob.inter) <- all.vars(formula.inter)} else {
      names(fit$coef.inter) <- names(fit$se.rob.inter) <- colnames(XinterMat)}
  class(fit) <- "LongitSC"
  fit
}
