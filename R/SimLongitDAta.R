...
J <- 3
n.sample <- 5
#SimLongitData <- function(n.sample, J,  gamma, alpha0.nt, alpha0.t, alpha0.or, alpha.nt, alpha.t, alpha.or, beta.nt, beta.t, beta.or)
SimLongitData <- function(n.sample, J,  gamma,  alpha.nt, alpha.t, alpha.or, beta.nt, beta.t, beta.or)
{
 p <- length(beta.nt) 
 X <- matrix(nr = n.sample, nc = p, rnorm(n.sample*p))
 YNT <- YT <- matrix(nr = n.sample, nc = J, 0)
 # p.nt <- expit(alpha0.nt + alpha.nt[1] + X%*%beta.nt)
 # p.t <- expit(alpha0.t + alpha.t[1] + X%*%beta.t)
 # OR <- exp(alpha0.or + alpha.or[1]+ X%*%beta.or)
 ...
 for(j in 2:J)
 {
  ...
  {
#  probs.T.only <- expit(alpha0.t + alpha.t[j] + X[at.risk.T.only, ]%*%beta.t + gamma)
  ...
  {
  # p.nt.both <- expit(alpha0.nt + alpha.nt[j] + X[at.risk.both, ]%*%beta.nt)
  # p.t.both <- expit(alpha0.t + alpha.t[j] + X[at.risk.both, ]%*%beta.t)
  # OR.both <- exp(alpha0.or + alpha.or[j]+ X[at.risk.both, ]%*%beta.or)
