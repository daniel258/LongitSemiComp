...
#' @author Daniel Nevo
#' @export
LongitSCparamInter <- function(longit.data, times = NULL, formula.NT, formula.T, formula.OR = NULL, formula.inter, data,
                     epsOR = 10^(-10), init = NULL, maxit.optim = 50000)
{
  ...
  res.opt <- optim(par = init, fn = ParamLogLikWithInter, gr = GradParamLogLikWithInter, hessian = T,
    ...
    fit$coef.inter <- fit$est[(1 + 3*K + pNT + pT + pOR + 1):(1 + 3*K + pNT + pT + pOR + pInter)]
    ...
    fit$se.inter <- fit$se[(1 + 3*K + pNT + pT + pOR + 1):(1 + 3*K + pNT + pT + pOR + pInter)]
  ...
  if (pInter==1) {
    names(fit$coef.inter) <- names(fit$se.inter) <- all.vars(formula.inter)} else {
      names(fit$coef.inter) <- names(fit$se.inter) <- colnames(XinterMat)}
  class(fit) <- "LongitSC"
  fit
}
